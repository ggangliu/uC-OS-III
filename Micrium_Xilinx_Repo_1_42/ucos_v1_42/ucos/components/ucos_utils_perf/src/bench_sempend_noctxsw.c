...
*                                          Evaluation Board
*
* Filename      : bench_sempend_noctxsw.c
* Version       : V1.00
* Programmer(s) : JBL
*********************************************************************************************************
* Note(s)       : none.
...
void bench_sempend_noctxsw ()
{
    OS_ERR  os_err;

    OSSemCreate(&sem1, "", 10u, &os_err);

    BenchResInit(&res);

    res.BenchName = "Semaphore Pend without Context Switch (Task)";
...
static void Task1 (void)
{
    ...
    for(iter_cnt = 0; iter_cnt < BENCH_CFG_ITER_CNT + BENCH_CFG_WARMUP_ITER_CNT; iter_cnt++) {
        t1 = BENCH_TS_GET();
        OSSemPend(&sem1, 0u, 0u, DEF_NULL, &os_err);
        t2 = BENCH_TS_GET();
        t = t2 - t1;
        if(iter_cnt >= BENCH_CFG_WARMUP_ITER_CNT) {
            BenchResUpdate(&res, t);
        }
        OSSemPost(&sem1, 0u, &os_err);
    }

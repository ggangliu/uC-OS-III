...
void bench_memcpy_libc ()
{
    OS_ERR  os_err;

    BenchResInit(&res);

    res.BenchName = "Memory copy (libc)";
    ...
    BenchResFin(&res);
    BenchResPrint(&res);
    UCOS_Printf("Average bandwidth %d\r\n", (CPU_INT32U)((double)BENCH_CFG_MEMCPY_SIZE / ((double)(res.Avg) / (double)CPU_TS_TmrFreq_Hz )) / (1024 * 1024));
...
static void Task1 (void)
{
    ...
    for(iter_cnt = 0; iter_cnt < BENCH_CFG_MEMCPY_ITER_CNT + BENCH_CFG_MEMCPY_WARMUP_ITER_CNT; iter_cnt++) {
        t1 = BENCH_TS_GET();
        memcpy((void *)BENCH_CFG_MEMCPY_DEST, (void *)BENCH_CFG_MEMCPY_SRC, BENCH_CFG_MEMCPY_SIZE);
        t2 = BENCH_TS_GET();
        t = t2 - t1;

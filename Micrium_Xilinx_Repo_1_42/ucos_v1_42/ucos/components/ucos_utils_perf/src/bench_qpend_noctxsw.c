...
*                                          Evaluation Board
*
* Filename      : bench_qpend_noctxsw.c
* Version       : V1.00
* Programmer(s) : JBL
*********************************************************************************************************
* Note(s)       : none.
...
void bench_qpend_noctxsw ()
{
    ...
    OSSemCreate(&sem1, "", 0u, &os_err);
    OSSemCreate(&sem2, "", 0u, &os_err);

    OSQCreate(&queue, "", BENCH_CFG_ITER_CNT + BENCH_CFG_WARMUP_ITER_CNT + 10u, &os_err);

    BenchResInit(&res);

    res.BenchName = "Q Pend without Context Switch (Task)";
...
static void Task1 (void)
{
    ...
    for(i = 0; i < BENCH_CFG_ITER_CNT + BENCH_CFG_WARMUP_ITER_CNT; i++) {

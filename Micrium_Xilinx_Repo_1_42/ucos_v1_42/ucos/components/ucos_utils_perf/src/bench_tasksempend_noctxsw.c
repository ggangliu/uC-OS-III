...
*                                          Evaluation Board
*
* Filename      : bench_tasksempend_noctxsw.c
* Version       : V1.00
* Programmer(s) : JBL
*********************************************************************************************************
* Note(s)       : none.
...
void bench_tasksempend_noctxsw ()
{
    OS_ERR  os_err;

    BenchResInit(&res);

    res.BenchName = "Task Semaphore Pend without Context Switch (Task)";
...
static void Task1 (void)
{
    ...
    OSTaskSemSet(&Task1TCB, BENCH_CFG_ITER_CNT + BENCH_CFG_WARMUP_ITER_CNT + 10u, &os_err);

    for(iter_cnt = 0; iter_cnt < BENCH_CFG_ITER_CNT + BENCH_CFG_WARMUP_ITER_CNT; iter_cnt++) {
        t1 = BENCH_TS_GET();
        OSTaskSemPend(0u, 0u, DEF_NULL, &os_err);
        t2 = BENCH_TS_GET();
        t = t2 - t1;

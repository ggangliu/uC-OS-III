...
*                                          Evaluation Board
*
* Filename      : bench_flagpend_noctxsw.c
* Version       : V1.00
* Programmer(s) : JBL
*********************************************************************************************************
* Note(s)       : none.
...
void bench_flagpend_noctxsw ()
{
    OS_ERR  os_err;

    BenchResInit(&res);

    res.BenchName = "Flag Pend without Context Switch (Task)";
...
static void Task1 (void)
{
    ...
    for(iter_cnt = 0; iter_cnt < BENCH_CFG_ITER_CNT + BENCH_CFG_WARMUP_ITER_CNT; iter_cnt++) {
        t1 = BENCH_TS_GET();
                OSFlagPend(&flag, DEF_BIT_00, 0u, OS_OPT_PEND_FLAG_SET_ANY, DEF_NULL, &os_err);
        t2 = BENCH_TS_GET();
        t = t2 - t1;

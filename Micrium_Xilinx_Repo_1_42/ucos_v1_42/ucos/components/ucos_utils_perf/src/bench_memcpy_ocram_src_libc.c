...
*                                          Evaluation Board
*
* Filename      : bench_memcpy_ocram_src_libc.c
* Version       : V1.00
* Programmer(s) : JBL
*********************************************************************************************************
* Note(s)       : none.
...
void bench_memcpy_ocram_src_libc ()
{
    OS_ERR  os_err;

    BenchResInit(&res);

    res.BenchName = "Memory copy OCRAM src (libc)";
...
static void Task1 (void)
{
    ...
    for(iter_cnt = 0; iter_cnt < BENCH_CFG_MEMCPY_ITER_CNT + BENCH_CFG_MEMCPY_WARMUP_ITER_CNT; iter_cnt++) {
        t1 = BENCH_TS_GET();
        memcpy((void *)BENCH_CFG_MEMCPY_DEST, (void *)BENCH_CFG_MEMCPY_OCRAM_ADDR, BENCH_CFG_MEMCPY_OCRAM_SIZE);
        t2 = BENCH_TS_GET();
        t = t2 - t1;
